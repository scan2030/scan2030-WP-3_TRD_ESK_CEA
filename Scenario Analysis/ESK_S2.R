...
par_mod <- define_parameters(
  ...
  p_death = case_when(
    model_time <= 6 ~ 0.001772008,  
    model_time >= 7 & model_time <= 13 ~ 0.000921875,  
    model_time >= 14 & model_time <= 19 ~ 0.000706614, 
    model_time >= 20 & model_time <= 26 ~ 0.000590032, 
    model_time >= 27 ~ 0.000513965    
  )
)
  ...
  "3rd_Response" = define_state(
    cost = discount_cost(
      ifelse(model_time == 1,
             ...
             ifelse(model_time >= 2 & model_time <= 3,
...
cost_by_cycle <- get_values(result, type = "cost")
qaly_by_cycle <- get_values(result, type = "effect")
