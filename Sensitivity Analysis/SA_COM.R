...
mod_COM <- define_strategy(
  ...
  "3rd_TX" = define_state(
    cost = discount_cost(c_3rdOP + c_3rdIP + c_SOPC + c_COM, d_rate, model_time),
  ...
  "3rd_Response" = define_state(
    cost = discount_cost(c_3rdOP + c_SOPC + c_COM, d_rate, model_time),
...
dsa <- define_dsa(
  ...
  rr_COM_remission, 0.82, 2.91,
  rr_COM_response, 1, 1.78,
...
name_map <- c(
  ...
  "rr_COM_remission" = "RR (COM): Remission",
  "rr_COM_response" = "RR (COM): Response",
...
ggplot(dsa_summary, aes(x = .icer, y = reorder(.par_names_label, diff), color = .col_icer)) +
  ...
  labs(
    title = "Esketamine vs. Combination Therapy",
    x = "ICER (USD/QALY)",
    y = NULL
  ) +
...
ggsave(
  filename = "dsa_icer_plot_COM.svg", 
  plot = last_plot(),             
  path = "output",                 
