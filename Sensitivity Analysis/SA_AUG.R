...
dsa <- define_dsa(
  ...
  rr_AUG_remission, 1.12, 1.47,
  rr_AUG_response, 1.17, 1.38,
...
name_map <- c(
  ...
  "rr_AUG_remission" = "RR (AUG): Remission",
  "rr_AUG_response" = "RR (AUG): Response",
...
ggplot(dsa_summary, aes(x = .icer, y = reorder(.par_names_label, diff), color = .col_icer)) +
  ...
  labs(
    title = "Esketamine vs. Augmentation Therapy",
    x = "ICER (USD/QALY)",
    y = NULL
  ) +
...
ggsave(
  filename = "dsa_icer_plot_AUG.svg", 
  plot = last_plot(),             
  path = "output",                 
