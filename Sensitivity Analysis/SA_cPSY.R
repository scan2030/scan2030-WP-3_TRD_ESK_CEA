...
dsa <- define_dsa(
  ...
  rr_cPSY_remission, 1.46, 2.52,
  rr_cPSY_response, 1.2, 2.69,
  rr_cPSY_rem2rel, 0.74, 0.97,
  rr_cPSY_res2rel, 0.74, 0.97,
...
name_map <- c(
  ...
  "rr_cPSY_remission" = "RR (cPSY): Remission",
  "rr_cPSY_response" = "RR (cPSY): Response",
  "rr_cPSY_rem2rel" = "RR (cPSY): Remission to Relapse",
  "rr_cPSY_res2rel" = "RR (cPSY): Response to Relapse",
...
ggplot(dsa_summary, aes(x = .icer, y = reorder(.par_names_label, diff), color = .col_icer)) +
  ...
  labs(
    title = "Esketamine vs. Psychotherapy plus oral AD",
    x = "ICER (USD/QALY)",
    y = NULL
  ) +
...
ggsave(
  filename = "dsa_icer_plot_cPSY.png",  
  plot = last_plot(),             
  path = "output",                
