...
dsa <- define_dsa(
  ...
  rr_mPSY_remission, 0.58, 1.09,
  rr_mPSY_response, 0.58, 1.09,
  rr_mPSY_rem2rel, 0.84, 1.25,
  rr_mPSY_res2rel, 0.84, 1.25,
...
name_map <- c(
  ...
  "rr_mPSY_remission" = "RR (mPSY): Remission",
  "rr_mPSY_response" = "RR (mPSY): Response",
  "rr_mPSY_rem2rel" = "RR (mPSY): Remission to Relapse",
  "rr_mPSY_res2rel" = "RR (mPSY): Response to Relapse",
...
ggplot(dsa_summary, aes(x = .icer, y = reorder(.par_names_label, diff), color = .col_icer)) +
  ...
  annotate("text", x = 50000, y = 2.5, label = "WTP = USD 50,000/QALY", angle = 90, 
           vjust = -0.5, hjust = -0.1, size = 3, color = "#011627", fontface = "bold") +
  annotate("text", x = 150000, y = 2.5, label = "3xWTP = USD 150,000/QALY", angle = 90, 
           vjust = 3, hjust = -0.1, size = 3, color = "#011627", fontface = "bold") +
  scale_color_manual(values = c(">" = "#F0868C", "<" = "#70A5D9")) +
  scale_x_continuous(labels = scales::comma) +
  labs(
    title = "Esketamine vs. Psychotherapy alone",
    x = "ICER (USD/QALY)",
    y = NULL
  ) +
...
ggsave(
  filename = "dsa_icer_plot_mPSY.svg", 
  plot = last_plot(),             
